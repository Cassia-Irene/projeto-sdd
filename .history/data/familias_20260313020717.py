...
def buscar_beneficiarios_reais(quantidade=40):
    ...
    while len(lista_final) < quantidade:
        url = f"https://api.portaldatransparencia.gov.br/api-de-dados/novo-bolsa-familia-sacado-beneficiario-por-municipio?codigoIbge={COD_IBGE_SLZ}&mesAno=202401&pagina={pagina_atual}"
        ...
        try:
            ...
            break  # Para evitar loop infinito em caso de erro ou falta de dados
...
def classificar_inseguranca(renda, tem_privacao):
    ...
    elif renda <= 0.6 and tem_privacao: return "Moderada"
...
def gerar_familias(total_alvo=80):
    ...
    i = 0
    while i < total_alvo:
        # Seleção de bairro com controle (máx 10 por bairro)
