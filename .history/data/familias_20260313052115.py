...
TIPOS_MORADIA = [
("Apartamento (Privado)", 0.15, 0),  # Prédios de classe média/alta (Ex: Renascença, Ponta d'Areia, Cohama)
    ("Alvenaria (Regular)", 0.50, 0),    # Casas padrão em bairros consolidados (Ex: Cohatrac, Anil)
    ("Conjunto Habitacional", 0.15, 0), # Apartamentos/Casas populares (Ex: MCMV, habitacionais antigos)
    ("Casa de Madeira", 0.08, 1),      # Estrutura frágil (periferias e ocupações recentes)
    ("Taipa/Barro", 0.07, 2),          # Construção precária (áreas rurais ou periferia extrema)
    ("Palafita", 0.05, 4)              # Risco Máximo (áreas de maré: Ex: Ilhinha, Jaracaty, Coroadinho)
]
...
def _sortear_renda():
    ...
    r = random.random()
    acumulado = 0.0
    for minimo, maximo, peso in FAIXAS_RENDA:
        acumulado += peso
        if r <= acumulado:
    ...
    while len(lista_final) < quantidade:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"⚠️ Erro na página {pagina_atual}: Status {response.status_code}")
                break
...
def gerar_familias(total_alvo=80):
    ...
    print("🚀 Iniciando geração híbrida (Real + Fake)...")
