...
def buscar_beneficiarios_reais(quantidade=40):
    """
    Busca beneficiários reais que realizaram saques do Bolsa Família em São Luís.
    Este endpoint retorna a lista detalhada com Nome, NIS e CPF.
    """
    # Endpoint correto para listagem de indivíduos por município
    ...
    try:
        ...
        if response.status_code == 200:
            dados = response.json()
            print(f"📡 API: {len(dados)} beneficiários reais encontrados.")
