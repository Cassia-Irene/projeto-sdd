...
# ── Mapa de calor (folium — já era interativo) ────────────────────────────────
def _gerar_mapa_calor():
    ...
    for f in familias.values():
        ...
        if bairro_nome in coords_dict:
            ...
            heat_data.append([lat, lng, pesos.get(nivel, 0.1)])
...
def gerar_dashboard_html():
    print("⏳ Gerando dashboard interativo com Chart.js...")

    mapa_html  = _gerar_mapa_calor()
    linhas_tab = _gerar_linhas_tabela()

    # Serializa todos os dados para JS
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .filtro-grupo select {{
      ...
      border-radius: 6px; padding: 7px 10px; font-family: 'DM Sans', sans-serif; font-size: 12px;
    ...
    thead th {{
      font-family: 'Space Mono', monospace; font-size: 9px; text-transform: uppercase;
      letter-spacing: 1px; color: var(--muted); padding: 9px 12px; text-align: left;
    ...
    footer {{
      text-align: center; padding: 16px; font-family: 'Space Mono', monospace; font-size: 9px;
      color: var(--muted); border-top: 1px solid var(--border); margin-top: 8px; line-height: 1.6;
    }}
...
<footer>
  ...
  Dados: Portal da Transparência (NovaBF/2024) + sintéticos via Faker · Gráficos: Chart.js
</footer>
...
function limparFiltros() {{
  ...
  aplicarFiltros();
    ...
    print(f"✅ Dashboard interativo gerado: {caminho}")
    print("   📊 Gráficos renderizados com Chart.js (sem matplotlib)")
    print("   🗺️  Mapa de calor via Folium")