...
def _grafico_privacoes_acumuladas():
    ...
    # Conta quantos fatores de risco cada família acumula (0 a 4)
    ...
    rotulos = ['Nenhum', '1 fator', '2 fatores', '3 fatores', '4 fatores']
    valores  = [contagem[i] for i in range(5)]
    cores    = [PALETTE['seguro'], PALETTE['leve'], PALETTE['moderada'],
                PALETTE['grave'], '#c0392b']
    ...
    ax.set_title(
        'Acúmulo de Fatores de Risco por Família\n'
        '(Menor de 18 · Escolaridade Baixa · Doença · Saúde Mental)',
...
vuln_dict = {b['nome']: b.get('vulnerabilidade_territorial', 2) for b in bairros_coords}
...
for f in familias.values():
    ...
    nivel = f.get('inseguranca', 'Seguro')
    vuln  = vuln_dict.get(bairro_nome, 2)
    
    if bairro_nome in coords_dict:
        lat, lng = coords_dict[bairro_nome]
        peso_base = pesos_inseguranca.get(nivel, 0.1)
        fator_territorial = {3: 1.5, 2: 1.0, 1: 0.6}.get(vuln, 1.0)
        heat_data.append([lat, lng, peso_base * fator_territorial])
