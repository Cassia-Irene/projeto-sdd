...
def gerar_familias(total_alvo: int = 80) -> None:
    ...
    while i < total_alvo:
        ...
        tipo_casa, pts_casa = _sortear_moradia_por_territorio(vuln_territorial)
        ...
        """
        tem_menor_18: probabilidade base do SEDES, aumentada proporcionalmente ao tamanho da família 
        — famílias com 4+ membros têm chance maior de incluir crianças ou adolescentes.
        """
        
        prob_menor = PROBABILIDADES["tem_menor_18"]
        if membros >= 4:  # Famílias maiores têm mais chance de ter menores
            prob_menor = min(0.85, prob_menor + (membros - 3) * 0.15)
        tem_menor = random.random() < prob_menor
        
        # Demais fatores de risco: probabilidades do diagnóstico SEDES,
        # independentes do território (ocorrem em qualquer bairro).
        ...
        """
        # Auxílio: determinado após renda e território 
        — mais provável em bairros vulneráveis com renda baixa. Beneficiários reais sempre True.
        """
        recebe_auxilio = _sortear_auxilio(vuln_territorial, renda, recebe_auxilio_real = pessoa_real)
        ...
        familias[id_unico] = {
            ...
            "renda_sm":           renda,
            "membros":            membros,
            "renda_pc_sm":        round(renda / membros, 3),
            ...
            "inseguranca":        classificar_inseguranca(renda, membros, tem_menor, escolaridade_baixa, pts_casa, vuln_territorial, recebe_auxilio),
        }
